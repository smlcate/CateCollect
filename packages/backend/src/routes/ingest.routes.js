...
export default function ingestApi(knex) {
  ...
  // Safely resolve archived file path for a DB row
  function resolveArchivedPath(row) {
    const p = (row?.archived_path || row?.stored_path || '').toString().trim();
    if (!p) return null;
    const abs = path.isAbsolute(p) ? p : path.join(ARCHIVE_DIR, p);
    return abs;
  }
  ...
  router.get('/health', async (_req, res, next) => {
    try {
      ...
      res.json({ ok:true, interval_ms: pollMs, incoming_dir: incomingDir, inbox_count: inboxCount, last_processed: last || null });
    } catch (err) { next(err); }
  });

  // ---------------------------
  // GET /files  (list; ?flat=1 returns array)
  // ---------------------------
  ...
  // ---------------------------
  // POST /files/:id/reprocess  (Parse XML/EMS → upsert ccc_metadata)
  // ---------------------------
  router.post('/files/:id/reprocess', async (req, res, next) => {
    try {
      ...
      // Helper to dig values with multiple candidate paths
      const pick = (...paths) => {
      ...
      // Heuristics for common CCC-ish shapes — tweak as you learn your inputs
